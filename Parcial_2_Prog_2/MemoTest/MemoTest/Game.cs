...
namespace MemoTest
{
    public partial class Game : Form
    {
        ...
        public Game() //Escondemos los paneles que no queremos que sean visibles y metemos los que si
        {
            InitializeComponent();
            panelEndGame.Hide();
            CreateGameEasy();
        ...
        private void Game_Load(object sender, EventArgs e) //Escondemos los paneles que no queremos que sean visibles y metemos los que si
        {

        }

        public void CreateGameEasy()
        {
            ButtonGame[,] buttonGames = nButtonGame.CreateEasyLevel(Program.Category); // Este metodo le devuelve una matriz de ButtonGames con valores ya inicializados

            for (int i = 0; i < 3; i++)
            {
                for (int j = 0; j < 4; j++) // Inserta cada una de las ButtonBox dentro de un TableLayoutPanel
                {
                    ...
                    GridEasyLeft.Controls.Add(buttonGames[j, i], j, i); // Agrega el buttonbox a la matriz
                }
            }
        ...
        public void GameButtonClick(object sender, EventArgs e)
        {
            ...
            if (fl == 1)
            {
                foreach (ButtonGame b in GridEasyLeft.Controls)
                {
                    if (b.LastFlipped == true && b.Value == bt.Value && b != bt) // Este bool se usa para saber si se acerto en los valores elejidos
                    {
                        b.Flipped = true;
                        bt.Flipped = true;
                        ...
                        equals = true; 
                        if (FlippedPairs == 6) panelEndGame.Show();  // Si es que ya adivino 12, significa que termino el Juego
                        break;
                    }
                }
                ...
                if (equals == false)  // Si es que no le pego a la variable se ejecuta lo siguiente
                {
                    foreach (ButtonGame b in GridEasyLeft.Controls)
                    {
                        b.Enabled = false; // Esto es necesario para que mientras 2 cajas estan dadas vueltas, uno no pueda empezar a dar vuelta 
                    }
                    ...
                    {
                        foreach (ButtonGame b in GridEasyLeft.Controls) // Hace lo mismo que la funcion de arriba solo que al revez, esconde ambos botones, les pone el LastFlipped = false, etc
                        {
                            b.Enabled = true;
        ...
        {
            Close();
            //Application.Run(new Game()); // No funiono
        }

        //public void PairClick(object sender, EventArgs e)
        //{
        //    if ((FlippedPairs %% 2 == 0 && LastFlipped == null) || (FlippedPairs %% 2 == 1 && LastFlipped != null))
        //    {
